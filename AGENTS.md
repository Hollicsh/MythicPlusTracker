...
| Look up a WoW API signature | `bash Tools/wow-api-lookup.sh [--source wiki\|framexml] <name>` | `.github/skills/wow-api-lookup/SKILL.md` — https://warcraft.wiki.gg for prose and examples, https://www.townlong-yak.com/framexml/live for Blizzard's own build-pinned definitions when exact types or a patch-to-patch change need verifying |
